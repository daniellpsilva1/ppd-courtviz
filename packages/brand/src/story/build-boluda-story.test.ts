...
describe("buildBoludaStory frozen metrics", () => {
  it("matches benchmark frozen payload values", () => {
    ...
    expect(story.frozenMetrics.hostPointsWonPct).toBe(Math.round((hostWin.rate ?? 0) * 1000) / 10);
    expect(story.frozenMetrics.guestPointsWonPct).toBe(Math.round((guestWin.rate ?? 0) * 1000) / 10);
    expect(story.frozenMetrics.hostFirstServeInPct).toBe(Math.round((hostFS.rate ?? 0) * 1000) / 10);
    expect(story.frozenMetrics.guestFirstServeInPct).toBe(Math.round((guestFS.rate ?? 0) * 1000) / 10);
    expect(story.frozenMetrics.hostBreakConvPct).toBe(Math.round((hostBP.rate ?? 0) * 1000) / 10);
    expect(story.frozenMetrics.guestBreakConvPct).toBe(Math.round((guestBP.rate ?? 0) * 1000) / 10);
    ...
    expect(story.frozenMetrics.hostTopZoneWinPct).toBe(
      Math.round((hostZones[0]!.winRate ?? 0) * 1000) / 10,
    );
  });
});
